...
def start_mqtt_broker(port):
    ...
    broker_process = subprocess.Popen(['mosquitto', '-p', str(port), '-v'],
    ...
    try:
        ...
        while (time.time()-start_time) < START_BROKER_TIMEOUT:
            ...
            if MOSQUITTO_PORT_IN_USE in error_message:
                raise MQTTBrokerPortNotAvailible(f'Could not start MQTT broker, port {port} already in use.')
            ...
            # Catch all other errors and raise exception
                ...
                raise MQTTBrokerError(f'MQTT broker exited with message: "{error_message}"')

        #TODO: add timeout for starting the broker
        raise MQTTBrokerError(f'Starting MQTT broker timed-out after {START_BROKER_TIMEOUT} seconds.')
