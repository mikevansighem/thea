...
Class MQTTHardwareModule():
    ...
    def run(self):
        ...
        while True:
            ...
            if self.hardware_config == HARDWARE_CONFIG_VIRTUAL:
                ...
                raise NotImplementedError(f"{self.hardware_config} is not a supported hardware configuration.")
    ...
    def _setup_as_new():
        ...
        mqtt_client.subscribe(MQTT_HARDWARE_CONFIG_TOPIC.replace('+',INITIAL_CLIENT_NAME))
...
main():
    ...
    parser = argparse.ArgumentParser(description='Starts a (simulated) MQTT hardware module from the CLI.')
    parser.add_argument('-a','--adress', help='MQTT broker adress.', default=MQTT_ADRESS)
    parser.add_argument('-p','--port', help='MQTT broker port.', default=MQTT_PORT)
    parser.add_argument('-c','--config', help='Hardware configuration.', default=HARDWARE_CONFIG_VIRTUAL)
